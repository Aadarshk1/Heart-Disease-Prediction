    ...
    if request.method == "POST":
        ...
        if user:
            if user.is_staff:  # Check if the user is an admin
                login(request, user)
                error = "admin"  # Set error message to indicate successful admin login
            else:
                error = "not_admin"  # Set error message if the user is not an admin
        else:
            error = "invalid_credentials"  # Set error message if authentication fails
    d = {'error': error}
